...
var main = function(netfile){
  d3.json(netfile, function(error, graph) {
    ...
    function nodeClick(d, i) {
      ...
      var blue = '#0000ff';
      var black = '#000000';
      var cmap = {'#008000' : 'green', '#0000ff': 'blue', '#000000': 'black'};
      //node props
      var inode = d3.select(this);
      var node_color = inode.style('fill');
      console.log(node_color)
      if (player=='Player 1') {
        var sugg_fill_color = green;
      } else {
        var sugg_fill_color = blue;
      }
      //color reversion
      if (sugg_fill_color == node_color) {
            inode.style('fill', black);
            nodes[i]['color'] = 'black';
      } else {
        if (node_color == black) {
            inode.style('fill', sugg_fill_color);
            nodes[i]['color'] = cmap[sugg_fill_color];
        }
      }
    }
